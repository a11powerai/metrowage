...
function PayslipsContent() {
    ...
    const exportPayslipPDF = async (record: any) => {
        ...
        doc.text(`Assembly Earnings:             Rs. ${record.assemblyEarnings.toLocaleString()}`, 14, y); y += 4;
        record.assemblyLines.forEach((l: any) => { doc.text(`  → ${l.productName}: ${l.quantity} × Rs.${l.appliedRate} = Rs.${l.lineTotal.toLocaleString()}`, 16, y); y += 5; });
        y += 4;
        ...
        <div>
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-slate-400 text-sm mt-0.5">Detailed itemized payslips with assembly earnings breakdown</p>
                </div>
                {period && (
                                        ...
                                        <>
                                            <div className="pt-1 border-t border-white/10">
                                                ...
                                                <div className="mt-1.5 ml-2 space-y-1">
                                                    {record.assemblyLines.map((l: any) => (
                                                        <div key={l.id} className="text-xs text-slate-400 flex justify-between">
                                                            <span>{l.productName} – {l.quantity} units × Rs.{l.appliedRate}</span>
                                                            <span className="text-slate-300">Rs. {l.lineTotal.toLocaleString()}</span>
                                                        </div>
                                                    ))}
                                                </div>
                                            </div>
